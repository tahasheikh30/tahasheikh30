...
def parse_args():
    ...
    src.add_argument("--data", help="Path to a JSON file: {title, axes:[{label,value}]}")
    ...
    p.add_argument("--exclude", default="", help="Comma-separated languages to drop (--github mode)")
    p.add_argument("--curve", type=float, default=1.0, help="Value ** curve; 1.0=linear, <1 flattens spikes")
    p.add_argument("--values", action="store_true", help="Print the number next to each axis")
...
def build_axes_from_github(username, limit, exclude, curve):
    ...
    if not totals:
        print("No language data found (empty account, or all languages excluded).", file=sys.stderr)
        sys.exit(1)
...
def render_svg(title, axes, color, bg_text_color, grid_color, show_values, theme):
    ...
    out.append(f'<text x="{cx}" y="28" text-anchor="middle" font-family="JetBrains Mono, monospace" '
    ...
    for ring in (0.25, 0.5, 0.75, 1.0):
        ...
        out.append(f'<polygon points="{" ".join(pts)}" fill="none" stroke="{grid_color}" stroke-width="1" opacity="0.5"/>')
    ...
    for i, ax in enumerate(axes):
        ...
        out.append(f'<line x1="{cx}" y1="{cy}" x2="{x:.1f}" y2="{y:.1f}" stroke="{grid_color}" stroke-width="1" opacity="0.5"/>')
        ...
        out.append(f'<text x="{lx:.1f}" y="{ly:.1f}" text-anchor="{anchor}" dominant-baseline="middle" '
    ...
    out.append(f'<polygon points="{pts_str}" fill="{color}" fill-opacity="0.30" stroke="{color}" stroke-width="2"/>')
    for x, y in pts:
...
def main():
    ...
    if len(axes) < 3:
        print("Need at least 3 axes for a radar chart.", file=sys.stderr)
        sys.exit(1)
    ...
    for theme, cfg in themes.items():
        svg = render_svg(title, axes, cfg["color"], cfg["text"], cfg["grid"], args.values, theme)
